...
# file_item_path = os.path.join(os.getcwd(),"cvs\data4.csv")
#
# print(file_item_path)
#
# file_item_path = os.path.join(os.path.dirname(__file__),"cvs\data4.csv")
#
# print(file_item_path)

def read_data(user_id=None,email=None):
    filename = os.path.join(os.path.dirname(__file__), "cvs\data4.csv")
    # filename = "hello\cvs\data4.csv"
    with open(filename, "r", newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        items = []
        unknowns_email = None
        unknowns_user_id = None
        for row in reader:
            if user_id is not None:
                if (int(row["id"]) == int(user_id)):
                    return row
                else:
                    unknowns_user_id= user_id
            if email is not None:
                if (str(row["email"]) == str(email)):
                   return row
                else:
                    unknowns_email = email


        if unknowns_user_id is not None:
           return "User id {user_id} not Found".format(user_id=user_id)
        if unknowns_email is not None:
...
# print(read_data(user_id=5))
# print(read_data(email="sean@teamcfe.com"))
#
# print(read_data(user_id=9))
# print(read_data(email="gay@teamcfe.com"))